...
public:
	...
	WriteAccess operator[](const K& key) {
		...
		return { lock_guard(bucket.m), bucket.map_part[key] };
	}

	ReadAccess At(const K& key) const {
		const Bucket& bucket = buckets.at(GetBucketIndex(key));
		return { lock_guard(bucket.m), bucket.map_part.at(key) };
	}

	bool Has(const K& key) const {
		const auto& bucket = buckets[GetBucketIndex(key)];
		lock_guard<mutex> g(bucket.m);
		return bucket.map_part.find(key) != bucket.map_part.cend();
	}
	...
	MapType BuildOrdinaryMap() const {
		...
		for (auto& bucket : buckets) {
			lock_guard<mutex> g(bucket.m);
			result.insert(bucket.map_part.begin(), bucket.map_part.end());
		}
...
private:
	...
	size_t GetBucketIndex(K value) const {
