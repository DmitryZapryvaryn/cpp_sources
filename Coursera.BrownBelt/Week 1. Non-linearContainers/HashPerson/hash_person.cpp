...
struct AddressHasher {
	size_t operator() (const Address& address) const {
		...
		return COEF * COEF * str_hasher(address.city) +
			COEF * str_hasher(address.street) +
...
struct PersonHasher {
	size_t operator() (const Person& person) const {
		const hash<string> str_hasher;
		const hash<int> int_hasher;
		const hash<double> dbl_hasher;
		const AddressHasher addr_hasher;
		return COEF * COEF * COEF * str_hasher(person.name) +
			COEF * COEF * int_hasher(person.height) +
			COEF * dbl_hasher(person.weight) +
			addr_hasher(person.address);
	}
};
