...
string httpCodeToString(const HttpCode code) {
	switch (code) {
...
public:
	...
	friend ostream& operator << (ostream& output, const HttpResponse& resp) {
		output << "HTTP/1.1 " << static_cast<int>(resp.code_) << ' ' << httpCodeToString(resp.code_) << "\n";

		for (const HttpHeader& header : resp.headers_) {
			output << header << "\n";
		}
		output << "\n";
	...
	HttpResponse ServeRequest(const HttpRequest& req) {
		...
		if (req.method == "POST") {
			if (req.path == "/add_user") {
				...
				return result.SetCode(HttpCode::Ok).SetContent(to_string(comments_.size() - 1));
			}
				...
				if (banned_users.count(user_id) == 0) {
					...
					return result.SetCode(HttpCode::Ok);
				}
				else {
			...
			if (req.path == "/user_comments") {
				...
				return result.SetCode(HttpCode::Ok).SetContent(response);
			}
			else if (req.path == "/captcha") {
				return result.SetCode(HttpCode::Ok).SetContent("What's the answer for The Ultimate Question of Life, the Universe, and Everything?");
			}
		}
