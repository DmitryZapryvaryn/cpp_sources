...
namespace geo2d {
	...
	uint64_t DistanceSquared(Point p1, Point p2) {
		int64_t diff_x = static_cast<int64_t>(p1.x) - p2.x;
		int64_t diff_y = static_cast<int64_t>(p1.y) - p2.y;
	...
	bool Collide(Circle c1, Circle c2) {
		return DistanceSquared(c1.center, c2.center) <= Sqr<uint64_t>(static_cast<uint64_t>(c1.radius) + c2.radius);
	}
